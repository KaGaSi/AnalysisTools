...
timestep (i.e., time evolution) and the averages over all timesteps from a
supplied \tt{agg} file (see Section~\ref{sec:AggFile} for its format). It
calculates number, weight, and z averages as well as distribution functions of
aggregation sizes.
...
Per-timestep averages are written to the \tt{<avg>} and  distributions into the
\tt{<distr>} file. Overall averages are appended as comments (with commented
legend) to both \tt{<avg>} and \tt{<distr>} files.

\tt{DistrAgg} can also calculate distribution of composition for aggregates with
specified size(s) (\tt{-c} option). Two versions of a \enquote{composition
distribution} are generated. The first is the distribution of numbers of each
molecule type in the aggregates of that size (written into
\tt{<file>-<size>.txt}; these data can be plotted as a 3D graph). The second is
a distribution of ratios of all possible molecular pairs in those aggregates
(written into \tt{<file>_r-<size>.txt}). In both cases, the number distribution
for aggregates with aggregation number $A_\text{S}$, $F_{A_\text{S}}(i)$, is
defined as
%
\begin{equation}
...
they are all included in the \tt{<avg>} and \tt{<distr>} files. However, for all
other variables such as average aggregate mass or shape descriptors of
aggregates (calculated via the \tt{GyrationAggregates} utility) only the total
mass of all molecules provides physically meaningful results, so only properties
with the total aggregate mass are calculated.

% Note that aggregate mass is always taken as the total mass, e.g., in the
% above points 8) and 9), the mass of the one detected aggregate would be the
% sum of masses of all the molecules in the aggregate even though the size
...
Lastly, only a specified range of aggregate sizes can be taken into
...
\vspace{1em}
\noindent
\tt{DistrAgg <input.agg> <distr> <avg> <options>}

\noindent
\begin{longtable}{p{0.27\textwidth}p{0.674\textwidth}}
  \toprule
  \multicolumn{2}{l}{Mandatory arguments}\\
  \midrule
  \tt{<in.stru>} & input structure file\\
  \tt{<in.agg>} & input \tt{agg} file\\
  \tt{<distr>} & output file with distribution of aggregate
    sizes \\
  \tt{<avg>} & output file with per-timestep averages\\
  \toprule
  ...
  molecule(s)\\
  \tt{-c <file> <int(s)>} & save composition distribution for specified
    aggregate size(s) to \tt{<file>} file \\
...
\begin{enumerate}[nosep,leftmargin=20pt]
  \item \tt{<distr>} -- distributions of aggregate sizes
      ...
      \item second to last line: column headers for overall averages
        \begin{itemize}[nosep,leftmargin=10pt]
          \item \tt{<As>_n}, \tt{<As>_w}, and \tt{<As>_z} are
            ...
            \begin{itemize}
              \item similarly to \tt{F_w(As)} and \tt{F_z(As)}, two columns are
                provided for each of \tt{<As>_w} and \tt{<As>_z} for the two
                mass definitions
            \end{itemize}
    ...
    \end{itemize}
  \item \tt{<avg>} -- per-timestep averages
    \begin{itemize}[nosep,leftmargin=5pt]
      \item first two lines: AnalysisTools version and the command
      \item third line: column headers
      \begin{itemize}[nosep,leftmargin=10pt]
        ...
        \item the calculated data follow: number, weight, and z average
          ...
          respectively)
          \begin{itemize}
            \item similarly to \tt{<distr>} file, two columns are provided for
      ...
      \item fourth line: note explaining the two mass definitions
      \item data lines follow
      \item the last two lines are the same as in \tt{<distr>}
    \end{itemize}
