...
the slices. Or to put it more rigorously, a density profile for a bead type $i$
along an axis $\alpha$, $\rho(\alpha)$, is calculated as
\begin{equation}
...
of the simulation box of the thickness $\Delta\alpha$ along the $\alpha$-axis;
$L_\beta$ and $L_\gamma$ represent box side lengths along the two remaining
axes.
...
Also note that the same density profile (along with other properties) can also
be calculated via the Local utility (\ref{sec:Local}).
