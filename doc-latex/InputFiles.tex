...
structure file used to generate it. Any of the \fullref{sec:Aggregates} utilities can create the \tt{agg} file.
...
\tt{m2}, etc., in case of multiple molecule types). If unsure, the \fullref{sec:Info} utility can provide the names of beads and
molecules for the specific structure file.
...
To be recognized, the file must be named \tt{CONFIG} or have the \tt{.config}
extension. %}}}
...
dimensions and \tt{ATOM} records for bead information: the atom name field gives
the bead type, the residue name field gives the molecule type, and the coordinate
fields give the bead position. As \tt{pdb} files contain no topology, this format
can only be used as a structure file.
