...
As a structure file, any of the following formats can be used; simple example
files are provided in the \tt{Examples/InputFiles} directory. Most utilities can
load a specific structure file using the \tt{-i <file>} option; if the option is
missing, it is usually assumed the structure file is the same as the coordinate
file.
...
specify the bead/molecule types. While many input files support naming
beads and molecules, not all do (in some formats, these are optional); in
these cases, AnalysisTools names the bead types as \tt{b} (and \tt{b1}, \tt{b2},
etc., if more than one bead type is present) and molecule types as \tt{m} (and
\tt{m1}, \tt{m2}, etc., in case of multiple molecule types). See the individual
...
complete description and the \tt{Examples} directory for example \vtsf files (a
\vtf file contains a structure part followed by a coordinate part, but a \vsf
file contains only the structure part). In this format, every bead is defined on
its own line and all bonds all listed.
...
package}. This concise format defines species of beads and molecules, providing
number of beads/molecules of given species as well as their properties.
...
potential parameters are only used for generating new LAMMPS data files via,
e.g., the \tt{Info} utility (\cref{sec:Info}). The \tt{molecules} section can
also contain one extra part: \tt{impropers} for improper angles which has the
same format as the \tt{dihedrals} section (this was added because the
\href{https://lammps.sandia.gov/}{LAMMPS molecular dynamics simulator} and other
software distinguish between the two types).
...
comment on the \tt{Atoms} line as required by the LAMMPS software).
...
files via, e.g., the \fullref{sec:Info} utility; note that to run LAMMPS
...
This file format comes from the LAMMPS
...
\tt{vx vy vz} (bead velocities), and \tt{fx fy fz} (bead forces). Other
attribute is ignored. Note that if \tt{element} is missing, all beads are
considered of the same type called \tt{b}.
...
Note that the term aggregate also refers to free chains (i.e., unimers or fully
dissolved chains).
