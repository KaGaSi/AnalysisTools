...
\fullref{sec:AddToSystem} and \fullref{sec:Info}) and provides pre-defined
bead types, interaction parameters, and molecule templates. In all library
files, blank lines and comment lines (beginning with \texttt{\#}) are ignored;
the first non-comment, non-blank line in each index file is a count that is
skipped.

The directory must contain four index files:

\begin{itemize}[noitemsep]
  \item \tt{list_parameters.txt} --- bead types and self-interactions; data
    lines have the form \tt{<index> <name> <mass> <charge> <A> <Rc>}, where
    \tt{A} is the DPD repulsion amplitude and \tt{Rc} is the cutoff radius.

  \item \tt{list_bonds.txt} --- harmonic bond types; data lines have the form
    \tt{<bond_ID> <k> <r0>}, where \tt{k} is the spring constant and \tt{r0}
    is the equilibrium length. Fortran-style \tt{d}-exponent notation (e.g.\
    \tt{1.5d0}) is accepted for \tt{r0}.

  \item \tt{list_angles.txt} --- harmonic angle types; data lines have the
    form \tt{<angle_ID> <k> <theta>}, where \tt{k} is the spring constant and
    \tt{theta} is the equilibrium angle.

  \item \tt{list_cross_interactions.txt} --- pairwise DPD interactions between
    different bead types; data lines have the form
    \tt{<index> <name1> <name2> <A> <Rc>}.
\end{itemize}

Additionally, each molecule template is stored in its own file
\tt{<molname>.txt} within the library directory. The format is similar to the
\field file: the first two non-comment lines are an arbitrary key string
(ignored) and the number of beads, followed by bead lines of the form
\tt{<index> <bead_ID> <x> <y> <z>} (bead indices are 1-based). An optional
\tt{Bonds} section (the keyword on its own line, followed by a count line,
then \tt{<bond_ID> <bead_i> <bead_j>} lines) and an optional \tt{Angles}
section (same structure, with \tt{<angle_ID> <bead_i> <bead_j> <bead_k>}
lines) may follow. The file is terminated by \tt{End}. Molecule templates are
loaded by name via the \tt{-mol} option. %}}}
