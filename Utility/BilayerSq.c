...
const struct HelpHelp HelpDesc = {
  "BilayerSq calculates the 2D in-plane static structure factor S(qx,qy) and "
  "its radially-averaged 1D form S(q) for each bilayer leaflet, then averages "
  "both leaflets. Leaflets are assigned by comparing each head-bead z-position "
  "to the global midplane (z-COM of all specified head beads). The q-grid "
  "spans qx in [-qmax, qmax] and qy in [0, qmax] with qmax = maxqbin * delta_q "
  "(the negative-qy half is reconstructed by symmetry in the output). "
...
static const struct OptSpec opts[] = {
  ...
  {"-q", "<int>","max bins per axis, qmax = n*delta_q (default: 50)", OPT_EXTRA},
  {NULL}
}; //}}}
...
static void Calculation(SYSTEM *System, struct calc_data *cd) {
  ...
  for (int l = 0; l < 2; l++) {
    ...
    for (int i = 0; i < nh; i++) {
      if (cd->leaflet[i] != l) continue;
      double xi = cd->hx[i];
      double yi = cd->hy[i];
      for (int iq = -mqb; iq <= mqb; iq++) {
        double qx_xi = (double)iq * dq * xi;
        int ioff = (iq + mqb) * njq;
        for (int jq = 0; jq <= mqb; jq++) {
          double arg = qx_xi + (double)jq * dq * yi;
          cd->sfx[ioff + jq] += cos(arg);
          cd->sfy[ioff + jq] += sin(arg);
        }
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc - 1; i++) {
    if (strcmp(argv[i], "-q") == 0) {
      ...
      if (!IsNaturalNumber(argv[i + 1], &v)) {
        err_msg("argument to -q must be a positive integer");
        PrintErrorOption("-q");
        exit(1);
      }
      maxqbin = (int)v;
  ...
  double *sfy     = malloc(n_qpts  * sizeof *sfy);
  if (!hx || !hy || !hz || !leaflet || !sfx || !sfy)
    ErrorAlloc("BilayerSq temp arrays");

  double *sq0 = calloc(n_qpts, sizeof *sq0);
  double *sq1 = calloc(n_qpts, sizeof *sq1);
  if (!sq0 || !sq1) ErrorAlloc("BilayerSq sqxy arrays");

  struct OPT opt = { .join = join };
  struct calc_data cd = {
    .opt         = opt,
    .specs       = specs,
    .n_specs     = n_specs,
  ...
  if (!sq_avg) ErrorAlloc("sq_avg");
  for (int k = 0; k < n_qpts; k++)
    sq_avg[k] = (sq0[k] + sq1[k]) * scale;
  ...
  for (int iq = -maxqbin; iq <= maxqbin; iq++) {
    double qx = iq * delta_q;
    int ioff = (iq + maxqbin) * njq;
    for (int jq = 0; jq <= maxqbin; jq++) {
  ...
  int n_1d = (int)(sqrt(2.0) * maxqbin) + 2;
  double *sum1d   = calloc(n_1d, sizeof *sum1d);
  int    *cnt1d   = calloc(n_1d, sizeof *cnt1d);
  if (!sum1d || !cnt1d) ErrorAlloc("1d arrays");

  for (int iq = -maxqbin; iq <= maxqbin; iq++) {
    int ioff = (iq + maxqbin) * njq;
    for (int jq = 0; jq <= maxqbin; jq++) {
      int k = (int)sqrt((double)(iq * iq + jq * jq));
      if (k < n_1d) {
        sum1d[k] += sq_avg[ioff + jq];
        cnt1d[k]++;
      }
    }
  }
  ...
  if (!data) ErrorAlloc("1d data");
  int row = 0;
  for (int k = 1; k < n_1d; k++) {
    if (cnt1d[k] == 0) continue;
  ...
  free(sfx); free(sfy);
  free(sq0); free(sq1); free(sq_avg);
  free(sum1d); free(cnt1d);
  FreeSystem(&System);
